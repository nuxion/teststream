...
function LoadConf(redis) {
  ...
  this.stream = new ORMRedis(redis, {
    prefix: 'stream',
    type: 'hash',
    fields: {
        host: 'h',
  ...
  this.version = new ORMRedis(redis, {
    key: 'ver',
    type: 'set'
  });
  this.shard = new ORMRedis(redis, {
    key: 'shard',
    type: 'set'
  });
}
...
/**
 * getTasks
 * It gets all the tasks from redis.
 */
LoadConf.prototype.getTasks = function() {
  return this.tasks.exec.selectall();
}
